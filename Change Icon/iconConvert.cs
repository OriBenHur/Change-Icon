...
namespace Change_Icon
{
    public static class iconConvert
    {
        ...
        public static bool ConvertToIcon(string inputPath, string outputPath, int size = 16, bool preserveAspectRatio = false)
        {
            ...
            FileStream inStream = new FileStream(In, FileMode.Open);
            ...
            if (inputBitmap.Width > inputBitmap.Height)
            {
                ...
                bmp = new Bitmap(width, (width - height)/2);
            }
            ...
            else if (inputBitmap.Width < inputBitmap.Height)
            {
                height = inputBitmap.Height;
                width = inputBitmap.Width;
                bmp = new Bitmap((height - width)/2, height);

            }
            else
            {
                width = size;
                float nPercent = ((float)size / (float)inputBitmap.Width);
                height = (int)(inputBitmap.Height * nPercent);
            }
            inStream.Dispose();
            ...
            if (main.Width > main.Height)
            {
                ...
                Graphics q = Graphics.FromImage(img3);
            ...
            else if (main.Width < main.Height)
            {
                ...
                Graphics o = Graphics.FromImage(img3);
            ...
            else
            {
                ...
                Bitmap img3 = new Bitmap(_width, _height, PixelFormat.Format32bppArgb);
                Graphics q = Graphics.FromImage(img3);
                q.DrawImage(main, new Point(0, 0));
                q.Dispose();
                img1.Dispose();
                img2.Dispose();
                main.Dispose();
